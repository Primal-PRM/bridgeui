...
const gasPriceFromSupplier = async (web3, url, options = {}) => {
  try {
    ...
    if (url === 'gas-price-oracle') {
      ...
      options.logger.debug(json, 'Gas price result from bsc api')

    } else {
      return null
    }
    // const oracleGasPrice = json[options.speedType]
    options.logger.debug(oracleGasPrice, 'Gas price result ...........')
    const oracleGasPrice  = json.result[options.speedType];
    if (!oracleGasPrice) {
      options.logger &&
