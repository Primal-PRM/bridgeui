...
const generateSnapshot = async (side, url, bridgeAddress) => {
  ...
  const getPastEventsWithFallback = (contract, eventName, options) =>
    contract.getPastEvents(eventName, options).catch(async e => {
      if (e.message.includes('exceed maximum block range') || e.message.includes('block range is too wide')) {
  ...
  await new Promise(resolve => setTimeout(resolve, (side === 'home' ? BLOCK_TIME_HOME : BLOCK_TIME_FOREIGN) || 0))
